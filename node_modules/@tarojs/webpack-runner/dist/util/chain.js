...
const Bundler = require("@tarojs/plugin-sass/bundler");
...
const makeConfig = (config) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (sass.resource && sass.projectDirectory) {
        ...
        try {
            if (typeof resource === 'string') {
                const res = yield Bundler(resource, projectDirectory);
                bundledContent += res.bundledContent;
            }
            if (Array.isArray(resource)) {
                for (const url of resource) {
                    const res = yield Bundler(url, projectDirectory);
                    bundledContent += res.bundledContent;
                }
            }
        }
    ...
    return Object.assign(Object.assign({}, config), { plugins, sassLoaderOption: Object.assign(Object.assign({}, sassLoaderOption), { data: sassLoaderOption.data ? `${sassLoaderOption.data}${bundledContent}` : bundledContent }) });
});
...
const getModule = (appPath, { staticDirectory, designWidth, deviceRatio, enableExtract, enableSourceMap, styleLoaderOption, cssLoaderOption, lessLoaderOption, sassLoaderOption, stylusLoaderOption, fontUrlLoaderOption, imageUrlLoaderOption, mediaUrlLoaderOption, miniCssExtractLoaderOption, esnextModules = [], module, plugins }) => {
    ...
    const additionalBabelOptions = Object.assign(Object.assign({}, plugins.babel), { sourceMap: enableSourceMap });
